  ...
  function shipCollisionBonus() {
      var ship_xw = ship_x + ship_w,
       ...
       for (var i = 0; i < bonuses.length; i++){
        ...
        if (bonuses[i]!=undefined && bonuses.length>0) {
        ...
        else if (ship_xw < bonuses[i][0] + bonus_w && ship_xw > bonuses[i][0] && ship_y > bonuses[i][1] && ship_y < bonuses[i][1] + bonus_h) {
        bonuses.splice(i,1);
        score+= 30;
        bonusSong.play();
         }
        //Coin inférieur gauche
        else if (ship_yh > bonuses[i][1] && ship_yh < bonuses[i][1] + bonus_h && ship_x > bonuses[i][0] && ship_x < bonuses[i][0] + bonus_w){
        bonuses.splice(i,1); 
        score+= 30;
        bonusSong.play();
        } 
        //coin inférieur droit
        else if (ship_yh > bonuses[i][1] && ship_yh < bonuses[i][1] + bonus_h && ship_xw < bonuses[i][0] + bonus_w && ship_xw > bonuses[i][0]){
        bonuses.splice(i,1); 
        score+= 30;
        bonusSong.play();
        };
